...
const Header = () => {
  ...
  const menuItems = [
    ...
    { name: "Commentarium", path: "/commentarium" },
    { name: "Contactus", path: "/contactus" }
  ];

  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white backdrop-blur-sm border-b border-foreground/10" role="banner">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center space-x-8" aria-label="Navigazione principale">
            {menuItems.map((item) => (
              <Link
                key={item.name}
                to={item.path}
